...
- Fresh authorization sessions initially competed with database connections retained by the HTTP vote handlers during broadcast. Both cast and retract now release their request database session before awaiting fan-out. Cast materializes its typed response after notification processing and before closing the session; email tasks retain scalar arguments rather than attached ORM objects.
