...
**Most recent refresh:** 2026-05-28 (Phase 41 — Tech Debt Audit Refresh + Bundled Tier-1 Fixes). Net change since Phase 35:
- **Items 67, 70, 73, B6.4, audit-log-IP-forensics RESOLVED** by Phases 33-40a.
- **6 NEW Tier-1 fixes bundled in Phase 41:** stale Phase-18-B3 TODO removal, B8 redundant flush removal, B6 AuditLog.target_id policy doc, B7 Organization.name uniqueness doc, B3 WebSocket FE wiring decision doc (option 3 = leave-and-document), B2 _eligible_viewers_for_proposal promotion to backend/eligibility.py + canonical public_delegate_user_ids helper (Phase 37 B3 followup).
- **NEW Items 84-94 logged** (Phase 32-40 surfaced items not previously in audit; per-item disposition below).
- **Deferred to dedicated passes:** B1 demo_users persona-source refactor (UX design call across 3 demo orgs vs Login.jsx 6-button grid), B4/B5 column-drop migrations (no migration in P41 per D4), B9-B12 notifications cleanup + Railway-OOM + async demo-reset (all >1 hour), Item 42 vitest infrastructure setup, Item 54 follow-through prod-snapshot-dump PG smoke mode.

**Edit history:**

- 2026-05-28 (Phase 41 closeout — Tech Debt Audit Refresh + Bundled Tier-1 Fixes):
  - **Phase 32.2 Item 67 (response builder Optional-with-None pattern) RESOLVED by Phase 33.** Audit doc already tracked this resolution; carried forward here for completeness.
  - **Phase 33 Item 70 (multi-worker reset race) RESOLVED by Phase 35** (workers default reduced to 1) AND structurally hardened by Phase 40 B1 (`with_for_update()` on `_acquire_lock` SELECT — PG row lock serializes concurrent triggers even if WORKERS>1 ever re-enabled).
  - **Phase 34 Item 73 (require_org_membership SubOrg accept) RESOLVED by Phase 34.1 E4.**
  - **Phase 37 B6.4 (Railway SIGTERM to PID 1 only) RESOLVED by Phase 40a** (`start.sh` trap fix; SM worker's "Received signal" log now appears in shutdown traces).
  - **Phase 37 Tier-2 audit-log IP forensics RESOLVED by Phase 38 ride-along** (uvicorn `--proxy-headers --forwarded-allow-ips '*'` so X-Forwarded-For propagates into `request.client.host`).
  - **NEW Item 84 (Tier 3): Phase 34 Item 74 — F1 per-section save races on settings JSON.** PATCH /api/orgs/{slug} merges via dict.update; concurrent saves race (last-write-wins). Not load-bearing at single-admin scale. Suggested: ETag header or optimistic-lock version field if multi-admin signal surfaces.
  - **NEW Item 85 (Tier 3): Phase 34 Item 75 — OrgBible.sub_orgs (legacy string list) coexists with OrgBible.sub_orgs_structured (Phase 34 seeding source).** Future bible cleanup pass drops the string list. Effort: ~15 min once verified no bible reads from `sub_orgs`.
  - **NEW Item 86 (Tier 3): Phase 34.1 Item 79 — DelegateProfile.org_id filter pattern at ~15 sites in routes/delegates.py + routes/delegate_profiles.py.** Same shape as Phase 34.1 E3 — would return empty when accessed via sub-org slug. Not load-bearing for current pilot (delegate pages accessed via parent slug). Suggested: bundled sweep when next sub-org delegate work is queued.
  - **NEW Item 87 (Tier 3, Phase 34.4): Demo bible OrgMembership-on-sub-org workaround now redundant.** Phase 34 hotfix #1 added OrgMembership rows on sub-org Organization rows for sub-org members; Phase 34.1 E4 made middleware sub-org-aware so the workaround is no longer load-bearing for routing. Defense-in-depth stays in seed pipeline. Future cleanup pass can remove (cuts ~50 OrgMembership rows per reset).
  - **NEW Item 88 (Tier 3, Phase 34.4): `hoa_karen` not in Cedar Hollow personas allowlist.** Karen is in the bible (SubOrgMembership admin on cedar-court-condos) with `quick_login=False`, so the seed pipeline skips her in the personas JSONB. Adding her to the quick-login picker is a UI design call (changes /demo's persona-grid count from 8 to 9 for Cedar Hollow). Not a bug; deferred pending QA need to exercise Karen's perspective.
  - **NEW Item 89 (Tier 3, Phase 36): Approval-proposal `option_totals` is sparse.** Options with 0 approvals omitted from the trajectory's `option_totals` dict (matches `option_approvals` shape). Today's FE doesn't render zero-approval lines, so consistent. Future polish item if chart UI changes to render flat-zero lines.
  - **NEW Item 90 (Tier 3, Phase 37 closeout, ongoing): Pre-existing 27-test full-pytest failure mode on master.** Same 27 tests have been failing across Phases 36 → 41 (verified each pass). Categories: phase3a_permissions (2), phase_12_5 user_permissions_field (1), phase_29_1 persona_delegations (1), test_ranked_choice_voting (16, share one fixture path), test_seed_idempotency (5). Targeted suites per Phase 37+ closeouts all PASS; the 27 failures appear environment-specific (SQLite-vs-Postgres, fixture stability). Worth a dedicated diagnostic pass to either fix or quarantine. **Diagnostic-audit candidate; not a bundled Tier-1 fix candidate.**
  - **NEW Item 91 (Tier 3, Phase 38): demo_users endpoint inline allowlist.** Spec B1 attempted the refactor but the structural ambiguity (3 demo orgs × ~18 personas each vs Login.jsx 6-button grid) requires a UX design call — does the legacy /login quick-switch source from one specific demo org, aggregate all three, or get retired entirely with Demo.jsx as the canonical surface? Deferred to dedicated Login.jsx/Demo.jsx product pass. Spec B1 disposition reasoning logged in P41 closeout.
  - **NEW Item 92 (Tier 3, Phase 38): B1 list-endpoint per-proposal eligibility check is O(N).** Documented Phase 38 closeout note. Fine at pilot scale (Cedar Hollow ~30 proposals); profile-driven followup if perf shows up.
  - **NEW Item 93 (Tier 2, Phase 40 hotfix #1): Pillow MAX_IMAGE_PIXELS 2× factor is subtle.** Pillow docs describe the constant as the hard cap but `DecompressionBombError` only fires at 2× the configured value. Phase 40 hotfix #1 added explicit pre-load `width*height` check that fires at exactly the advertised threshold — preserve this pattern in any future Pillow-based upload handlers. Add to backend Pillow-usage convention doc when next touched.
  - **NEW Item 94 (Tier 3, Phase 33): TopicCreate schema silently drops `description` field per Pydantic default.** Item 71 in this doc previously. Switch to `model_config = {"extra": "forbid"}` if explicit reject desired. User-visible behavior change for older API clients (silent drop → 422); defer to a dedicated API-input-validation pass.
  - **Phase 41 Bundled Fix 1 (stale TODO removal):** 5 `# TODO Phase 18 B3: make required` comments removed from `backend/delegation_engine.py` (org_id retrofit complete per Phase 18 closeout; the TODO was never actionable). Commit `3e37225`.
  - **Phase 41 Bundled Fix 2 (B8):** `db.flush()` removed from `_auto_join_demo_org` in `routes/auth.py`. Caller (verify-email endpoint) commits at end of handler. Commit `30ff111`.
  - **Phase 41 Bundled Fix 3 (B6 + B7 docs):** SECURITY_REVIEW.md gets explicit policy sections for AuditLog.target_id (no FK by design — records what happened, including against now-deleted entities; future enrichment via details JSONB pattern) + Organization.name uniqueness (NOT NULL but not unique; slug is the unique routing key; design answer for collision is UI disambiguation not constraints). Commit `13f0720`.
  - **Phase 41 Bundled Fix 4 (B3 doc):** `docs/websocket_status.md` created. WebSocket `/ws/proposals/{id}` endpoint hardened by Phase 38 B2, but FE has zero callers (grep confirms). Three options enumerated (wire / delete / leave); Phase 41 chooses option 3 (leave-and-document) per spec D8 default — FE wiring is product-direction work, not hygiene. Commit `e320392`.
  - **Phase 41 Bundled Fix 5 (B2):** `_eligible_viewers_for_proposal` promoted from `routes/comments.py` to new `backend/eligibility.py`. Import sites updated in `routes/proposals.py` + `main.py` (WebSocket handler). `routes/comments.py` retains re-export under legacy name for back-compat. Behavior unchanged. Targeted tests 61/61 PASS. Commit `973fc0c`.
  - **Phase 41 Bundled Fix 6 (Phase 37 B3 followup):** Canonical `public_delegate_user_ids(db, *, topic_id=None)` helper added to `eligibility.py`. Both inline reimplementations replaced: `routes/users.py::delegation_tree` (no topic filter) + `routes/proposals.py::get_vote_graph` (per-topic loop). Defends against future drift if public-tier visibility values change. Targeted tests 24/24 PASS. Commit `5815711`.
...
- 2026-05-21 (Phase 33 closeout — Tech Debt Audit Refresh):
  ...
  - **Item: Legacy `/api/delegates/register` + `/api/delegates/register/{topic_id}` endpoints RESOLVED via deletion.** Phase 30 B1 had already retired the frontend caller (Settings.jsx links to the canonical per-org delegate-profile page); endpoints were dead code with no callers, no tests. Deletion was the load-bearing prerequisite for Item D1 (DelegateProfile.is_active drop).
  - **Item D1: `DelegateProfile.is_active` column drop RESOLVED.** Migration `b6d8e2f1a350` drops the column. All 14 `is_active.is_(True)` filter sites removed across 6 backend files (permissions.py, routes/delegates.py, routes/users.py, routes/delegations.py, routes/follows.py, routes/proposals.py). `is_active=True` constructor kwarg removed from seed_data.py, seed_pipeline.py, delegate_profiles.py + ~16 test files. `DelegateProfileOut` schema keeps `is_active: bool = True` as a literal default for external API consumer back-compat. Reversible: `down()` re-adds with `server_default=true()`.
  - **Item D2: `Topic.description` column drop RESOLVED.** Migration `b6d8e2f1a350` drops it. `TopicCreate` keeps `description` as ignored-extra (older clients send it; Pydantic silently drops). `TopicOut` field removed. Routes + seed pipeline write sites removed. Frontend Topics admin "Description" input + state removed. ~44 test references stripped across 25 test files. Reversible: `down()` re-adds with `server_default=""`.
  - **Item C1: `/api/users/me` "User not found" bug RESOLVED.** Root cause: `GET /api/users/{user_id}` matched literal string "me" as `user_id`, which then 404'd on DB lookup. Fix: explicit `GET /me` handler in `routes/users.py` before the generic `/{user_id}` route — returns `current_user`. Phase 33 T-cluster adds regression test.
  - **Item: Demo-reset scheduler investigation (Phase 33 C2) — surfaced observability gap, not a confirmed silent failure.** Investigation findings: (a) backend runs 4 uvicorn workers (`start.sh: --workers ${WORKERS:-4}`) each with independent `digest_loop` — CLAUDE.md's "single-instance scheduler" assumption is incorrect for prod. (b) Pre-Phase-33 the demo reset block had NO info-level logging on the skip OR success path; only the deep "Graph store refreshed after demo reset" line indicated success, and the tick `counts` dict carried no `demo_reset` field. (c) 1.5h of prod logs showed no demo_reset evidence but also no failure logs — couldn't confirm broken vs working from log evidence alone. (d) The only confirmed successful reset since Phase 23.1 was the manual trigger from Phase 32.2 closeout. Fix: instrumented `digest_scheduler.run_one_tick` to log demo reset outcomes and surface them in `counts["demo_reset"]`. Future ticks will be visible in the existing "tick complete" log. **NEW Item 70 (Tier 2):** multi-worker race on `is_demo_resetting` lock — 4 workers all attempting `_acquire_lock` rely on FOR UPDATE-style behavior that the current row-level read doesn't provide. Item D3 (async demo-reset endpoint) would consolidate to a single-worker pattern; deferred this pass.
  - **Item D3 (async demo-reset endpoint) DEFERRED.** Requires BackgroundTasks lifecycle + job-id tracking model + status-poll endpoint + script update. Estimated >2 hours. Re-dispatch as dedicated pass when next demo-content iteration needs to bypass the 15-min sync block.
  - **Item D4 (Phase 21 audit items 59-61) DEFERRED.** Each item is 1-2 hours; outside Phase 33's already-substantial scope. Recommend a dedicated "Notifications cleanup" pass when next user-facing notifications work is queued.
  - **NEW Item 71 (Tier 3):** TopicCreate schema's `description: str = Field(...)` field removed; older API clients sending `description` in the request body now have that field silently ignored (Pydantic default behavior allows unknown fields). If we want a 400 instead of silent drop, switch to `model_config = {"extra": "forbid"}`. Not load-bearing at v1 scale.
  - **NEW Item 72 (Tier 2):** Phase 33 B1 loud-failure guard catches resolver-org-mismatch at test time. Future code may legitimately want to call resolvers without an org (e.g., a CLI tool computing effective values offline); document the guard pattern in CLAUDE.md so callers know to either pre-load the org or use a sentinel value. Tier 2 because the guard surfaces the right design intent; only revisit if it becomes a friction point.
  - **Items 5 (sustained-majority floor breached) + 22 (NotificationBadge default-org coarse routing) + 28-29 (legacy `owner` cleanup) + 33 (help page back-links) + 47 (DELETE 204 implicit-None pattern) + 48 (public-read transparent-only delegates) + 54 (pg_smoke actual-upgrade structural blind spot) etc.** — already marked CLOSED in earlier edit-history entries; this Phase 33 walk confirmed no regression. Items remain in the doc with RESOLVED status for traceability rather than being deleted.
