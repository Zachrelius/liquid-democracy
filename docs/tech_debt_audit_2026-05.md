...
### Item 33: Help page back-links destination
- Source: PROGRESS.md Phase 11 tech debt #3
- Description: PolisHelp / VotingMethodsHelp / SustainedMajorityHelp Back-to links currently point at `/orgs` (org context-free). Could point at `history.back()` for smarter context-aware navigation.
- Question for Z: Want context-free `/orgs` (current behavior) or smarter `history.back()` (with fallback to `/orgs`)?
