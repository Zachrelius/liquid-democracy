...
def run_one_tick(db: Session) -> int:
    ...
    voting_proposals = (
        ...
        # Phase 73/74 — budget proposals are excluded from the sustained-
        # majority worker. Stable-result is rejected at create for budget
        # methods (§4), and the worker's snapshot/evaluate path reads binary/
        # multi-option ballot shapes only — not an allocation/project ballot.
        .filter(models.Proposal.voting_method.notin_(["budget_allocation", "budget_project"]))
        .all()
    )
    processed = 0
