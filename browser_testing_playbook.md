...
- **R9** — Cross-scope inheritance — sub-org member with non-member delegate sees correct "your vote" status on sub-org proposal (chain behavior, not silent failure). **Status: PASS as of Phase 8.6 (2026-04-29).** voter02 has Economy → econ_bob delegation (seeded in Phase 8.6 Item 2); the trunk-based-development proposal references Economy at relevance 0.4 (Phase 8.5 Session 4 wiring); engine resolves econ_bob as voter02's delegate, detects he has no ballot (not an Engineering member, excluded by `eligible_voter_ids_for_proposal`), accept_sub chain-behavior fallback fires. `/api/proposals/{id}/my-vote` returns `message: "Your delegate Bob the Economist has not voted. Chain behavior: accept_sub."` Frontend cross-scope copy logic in `ProposalDetail.jsx` renders the spec copy on this case (source-reviewed; UI not browser-traced live because demo persona-list UI doesn't expose voter02 as a quick-login button — `Demo.jsx` button list is hardcoded narrower than `DEMO_USERNAMES`).
