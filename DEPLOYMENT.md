...
## Reading Railway logs during a 502 incident (Phase 13.1 W-RUNBOOK)
...
The Phase 13 incident (2026-05-04) is the canonical example: backend went 502 immediately after a deploy and stayed down for ~35 minutes before the team reverted. The team had no Railway log access during the incident, which made the failure mode unobservable in real time. This section is the runbook so the next time it happens, the team can see what's actually breaking.

### Where the logs are

**Railway dashboard:** project → service (the backend service) → "Deployments" tab → click the failing deployment → "Logs" sub-tab. This shows the container's stdout/stderr streamed live, with a "Tail" toggle for following new lines. Filter by time range when investigating an old deploy.

**Railway CLI** (faster, scriptable):

```bash
railway logs --service <backend-service-name>           # live tail
railway logs --service <backend-service-name> --tail    # alias, same thing
```

To grab recent logs without tailing forever, redirect to a file and Ctrl-C after a few seconds:

```bash
railway logs --service <name> > /tmp/backend.log &
sleep 10 && kill %1
```

The Railway CLI authenticates via `railway login` and pins to a project via `railway link` (or `railway run --service ...` to invoke ad-hoc commands).

### What to look for during a 502 incident

Top-to-bottom on the failing container's logs:

1. **Did `start.sh` start at all?** Look for the first `start.sh` line — typically the alembic check ("Alembic-stamped DB detected — applying pending migrations…" OR "Fresh database detected — bootstrapping via create_all + stamp head."). Absent → the container never invoked the start script (Dockerfile / image build issue, not a deploy issue).

2. **Did alembic upgrade succeed?** Look for `alembic` output. A migration error here (DDL collision, missing prior revision, FK constraint violation against existing data) ends start.sh with non-zero exit due to `set -e`, container restarts in a loop. Fix: revert the offending migration or hot-fix the migration's idempotency.

3. **Did the seed step run cleanly?** Only relevant if `IS_PUBLIC_DEMO=true`. A failing `seed_if_empty.py` exits with set -e and the container loops. Less common, but watch for "Public demo mode — ensuring demo seed data…" followed by a stack trace.

4. **Did the worker side-process start?** Look for `Starting sustained-majority worker…` followed by `Sustained-majority worker PID: <N>`. **The PID line prints even if the worker dies milliseconds later** (because `&` backgrounds the process; the PID echo doesn't wait for it). If the worker import-crashes immediately, you'll see a Python traceback nearby that does NOT contain the PID echo on a clean line — it'll be interleaved. **The worker dying does NOT take down uvicorn** (the bash `&` operator doesn't propagate exit codes to set -e), but the worker's failure may correlate with whatever broke uvicorn's startup chain.

5. **Did uvicorn reach the listening state?** Look for `INFO: Started server process` (one per worker) followed by `INFO: Application startup complete.` and `INFO: Uvicorn running on http://0.0.0.0:8000`. Without those lines the app never bound to the port. Causes: import-time exception in any module reachable from `main.py`, an exception inside `@app.on_event("startup")` (the startup hook), or a hang inside startup (e.g., a blocking DB call that times out).

6. **Are individual workers OK but health checks failing?** Look for repeated `GET /api/health HTTP/1.1` lines. Absent → Railway's health probes aren't reaching the container at all (port misconfiguration, container OOM-killed before binding). Present but returning 5xx → app is up but health endpoint is broken (rare).
...
A 502 with no `/api/health` response means **the container did not reach the `exec uvicorn` line**, OR uvicorn started but never reached `Application startup complete.`. The first three steps run in foreground with `set -e`; any failure exits the script and Railway loops the container. The `&` on the worker line decouples its lifecycle from start.sh, so worker death doesn't end start.sh.

Phase 13.1 added two defensive patterns to make startup more resilient:

- **Defensive import in `sustained_majority_worker.py`** — `from notification_emit import emit_notification` is wrapped in try/except so a downstream import failure can never crash the worker process. Pattern: try the import, log on failure, set a flag, guard the call site on the flag. (Lands in W-DEPLOY-2.)
- **Defensive scheduler launch in `main.py` startup hook** — `asyncio.create_task(digest_loop())` is wrapped in try/except so a scheduler-launch failure can never crash startup. The endpoints stay up; only the scheduled digest job goes silent. (Lands in W-DEPLOY-3.)

These patterns are the canonical shape for any future code reachable from start.sh's worker side-process or from the FastAPI startup hook.

### When to revert vs. hot-fix-forward

If the cause is obvious from the logs and the fix is one or two lines, hot-fix-forward (push, redeploy, observe). Examples: a typo in an environment variable name, a missing import, a guard clause that should have been in place.

If the cause requires more than ~10 minutes of investigation OR if the diagnosis isn't certain, **revert immediately** with `git revert -m 1 <merge-sha>` and push. Phase 13's revert took ~2 minutes from push to backend recovery; the diagnostic happens offline, and the next deploy goes through with proper sequencing. The sunk cost of the failed deploy is small compared to the cost of cascading hot-fix attempts.

The Phase 13.1 bisection pattern (split a feature pass into independent deploys with separable risk surfaces) is the longer-horizon fix; this runbook is for the in-the-moment incident.
