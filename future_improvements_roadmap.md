...
### 3. Public Delegate Pages — ✅ Complete (Phase 19, shipped 2026-05-10)

Phase 19 shipped the public-delegate identity surface end-to-end. New `OrgDelegateProfile` table holds per-org delegate identity (intro, page_visibility); existing `DelegateProfile` rows gain a per-topic `visibility` enum (`private` / `public` / `public_accepting`) plus `position_statement` and approval lifecycle timestamps; new `DelegateVoteRationale` table holds opt-in per-vote explanations; new `User.delegate_handle` provides the URL-surface identity. URL pattern: `/{slug}/delegates/{handle_or_username}` (public read with `effective_page_visibility` auth gate); browse page at `/{slug}/delegates` (org-scoped, sortable by delegation count + recent rationale ratio). Page-visibility ladder: `private` (drafting only) → `private_delegators` (approved followers in this org via Phase 18 `FollowRelationship.org_id` filter) → derived `public` (auto-activates when any topic is non-`private`). Effective visibility = `min(page_visibility, max(topic_visibility))` centralized in `OrgDelegateProfile.effective_page_visibility()`. Approval workflow gates the `public → public_accepting` topic transition by `delegate_application.approve` permission (already in `permission_registry.py` from Phase 12 Stage 1 — no new permission key); approvers see the application, approve with one click or deny with comment; applicant resubmits if denied. D15 hard-revert (`public/public_accepting → private`) auto-revokes public-origin delegations on that topic and notifies affected delegators via new `delegation_revoked_by_delegate` event; private-origin (follow-based) delegations are preserved via the centralized `_revoke_public_origin_delegations_on_topic` helper (uses DelegationIntent-row-existence-based detection per the spec/reality reconciliation since Wave 1 didn't add `Delegation.delegation_intent_id`; logged as audit Item 53 for future FK column add). Frontend: F1 own-management page with hard-revert confirmation dialog; F2 public delegate page; F3 inline rationale composer on proposal-detail; F4 browse page; F5 approver dashboard. Four new notification events: `delegate_application_submitted/approved/denied` + `delegation_revoked_by_delegate`. Help page added at `/help/public-delegates`. See spec `phase19_public_delegate_pages_spec.md` and PROGRESS.md Phase 19 entry. Six new audit items logged (Items 48-53; Item 48 fixed inline pre-merge; Items 49-52 are F1/F3/F5 API-gap follow-ups; Item 53 is the structural-fragility note on the missing FK column). Deferred per spec §"What this pass is NOT": delegate-to-delegate Q&A, follower comments on delegate profiles, formal endorsement systems.
...
### 4.6 Demo Daily Reset Infrastructure — ✅ Complete (Phase 23, shipped 2026-05-12)

Builds the technical foundation for the curated-demo experience. Three demo orgs (`demo-cedar-hollow` Cedar Hollow HOA, `demo-local-4021` AFSCME Local 4021, `demo-westgate-coalition` Westgate Tenants Coalition) get wiped and re-seeded daily from checked-in Python bible modules at `backend/demo_content/`. Migration `c7e8a3d419f5` adds `Organization.is_demo`, `is_demo_resetting`, `governance_type`, `display_order`, `personas` JSONB + 3 branding columns (`brand_color`, `brand_secondary_color`, `logo_url`) + `User.headshot_url` (branding deferred to Phase 24, columns ship now). The reset job hooks into Phase 13's existing notification scheduler (no new worker process); manual trigger `POST /api/admin/demo/reset` for ops. The snapshot generator emits Phase-22-compliant `option_totals` JSON for multi-option proposals. Filler-member generator (`backend/demo_content/filler_generator.py`) produces ~55 deterministic members per org via `(org_slug, member_index)`-seeded PRNG; ~30% delegate to public delegates for realistic follower counts. Cross-org user mapping (Stage 8 §5): Marcus/Dana/Janet each get one underlying User row + two OrgMemberships, so org-switching surfaces per-org delegate page content correctly. Frontend ships a `DemoOrgBanner` on demo-org pages + a Demo.jsx three-org-card rewrite consuming `GET /api/orgs/demo`. `POST /api/auth/demo-login` extended with optional `org_slug` for per-org persona allowlist (legacy single-org `{username}` path preserved during transition). Five new Tier-3 audit items (Items 65-69): multi-option snapshot tally heuristic, persona descriptions fallback to role, 5 bible event_types not in Amendment A template table, filler multi-option allocation not aimed at trajectory, filler comments deferred. See spec `phase23_demo_daily_reset_spec.md` + amendments `phase23_amendments_2026-05-12.md` + Stage 8 handoff `docs/demo/handoff_stage8.md` + PROGRESS.md Phase 23 entry.
