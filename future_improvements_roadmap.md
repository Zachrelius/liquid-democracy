...
- **Sustained-majority floor activation logic (resolved in Phase 9.8)** — pre-9.8 the floor-breach detector fired on the first negative vote when zero yes votes had been cast. Fixed in 9.8 via new `support_ever_established(snapshots, config)` helper: floor only activates after `support_fraction >= threshold` is reached at least once during the window. Existing tests that encoded the bug were updated to seed an establishing snapshot first.
...
- **Avatar storage on Railway-ephemeral filesystem** — Phase 9.8 ships profile pictures to `backend/uploads/avatars/{user_id}/{128|48}.jpg` served via FastAPI `StaticFiles`. Files persist within a deploy but are wiped on container restart. Acceptable for friend-pilot (5–15 users); migrate to Railway Volume or object storage (S3/R2) before broader pilot. The `users.avatar_url` DB column already stores a path string, so the migration only touches the upload/serve layer — not the schema.
- **`org_slug` is optional on `/api/users/search`** — Phase 9.9 added the parameter to scope delegate search to the active org (and 403's cross-org callers). Backward compat preserved: when `org_slug` is omitted, the endpoint still returns the global user list. All in-app callers were updated to pass `org_slug`, but legacy/admin tools or direct API users could still hit the unscoped path. Low-priority follow-up: make `org_slug` required and update any remaining callers (none found in the frontend at 9.9 ship time, but worth a recheck before requiring it).
- **Test depth audit (recommended for a follow-up mini-pass)** — Phase 9.6 and Phase 9.7 each surfaced "feature works at the API layer but the end-to-end user flow was never built" gaps from real-world pilot use (Phase 9.6: missing `send_invitation_email` call; Phase 9.7: missing frontend route, missing invitation-aware register/login, missing auto-join-skip guard). Both are Phase 4-era code paths that passed their original API-contract tests. The pattern is worth a small dedicated audit pass: enumerate every endpoint that involves an external-touching workflow (email links, OAuth callbacks, webhook receipts, file uploads with post-processing), check whether each has at least one test asserting the user-journey side-effect (not just the route response shape), and add the missing ones. Estimated 1-2 sessions; would catch any other latent gap of this shape before pilot users surface it.
