...
**Tests:** +1 in `test_phase_72c_topic_warn_and_drop.py` (Part C UUID-regression guard). Updated `test_phase_72_multi_proposal_import.py::test_array_per_item_topic_and_unknown` to assert topic name (not UUID) in resolution warning. Full suite: 2641 → **2642** passed / 0 failed. No migration. Bundle `index-BMKhI2qP.js`. Merge `e245b41`. Backend `/api/health` 200 confirmed.
