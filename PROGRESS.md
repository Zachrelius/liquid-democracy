...
## Phase 91a — Private Backend Network ✅ Complete (2026-07-13)

Spec: `phase91a_private_backend_network_spec.md`. Branch `phase-91a/private-backend-network`; implementation commit `f5910d1`; no-ff merge `ce427a0`.

Closed Phase 91's remaining public-ingress/proxy-trust issue. The frontend is now the only public application gateway and proxies `/api`, `/ws`, and `/uploads` to `http://backend.railway.internal:8000`. Nginx uses Railway's container resolver with a short TTL so backend-only redeploys do not strand the frontend on a stale private IP. It normalizes the edge-controlled first XFF address, scheme, and public host before forwarding; Uvicorn retains proxy-header support behind the private boundary and uses Railway's documented dual-stack bind form.

The cutover was staged with the old backend hostname available until every gate passed. Frontend private-route deployment `91183195-0f89-49f7-b8b1-9143c32211e5` succeeded. A live forged-XFF/X-Real-IP login probe recorded the real caller IP and none of the spoofed values. Backend-only redeploy `2eb59786-2844-4b39-be20-2afd2717a717` succeeded while the frontend deployment ID remained unchanged, proving request-time private DNS recovery. Railway service domain `backend-production-8014c.up.railway.app` was then removed; Railway reports no backend service/custom domains, the former hostname returns 404, and the main health endpoint remains 200.
...
Implementation `d7c3a27`; no-ff merge `8fe3fce`. Railway backend deployment `271e7926-305d-4fd6-8628-b1b2e1818de2` matched the merge and reported successful. Production smoke passed: homepage/readiness/scheduler/combined monitor all 200; the monitor reported `ok`, database capacity 5.67%, uploads 0.03%, zero recent 5xx, zero email-failure streak, current scheduler heartbeats, one eligible alert recipient, and no issues. GitHub Actions production-monitor run `29653540197` succeeded in 8 seconds and opened no incident. Closeout `4928f33` / no-ff merge `da2f28b` moved the workflow to the Node 24 `actions/github-script@v8` runtime; final manual run `29653634264` succeeded in 7 seconds with no annotations and no open `production-monitor` incident. Safe email-delivery behavior is covered by mocked transport integration plus the audited test-alert endpoint; no fake production outage was created. No new paid subscription or recurring service was added.

---

## Phase 98 — Layered Backup and Recovery 🟡 Stage 6 isolated restore proven (2026-08-14)

Spec: `phase98_layered_backup_and_recovery_spec.md`. Branch `phase-98/layered-backup-recovery`; implementation `83698f1`; initial no-ff merge `ab4356f`.

Stage 0 read-only inventory confirmed Railway remains on Hobby for the August 5–September 5 billing cycle. The authenticated UI still gates Postgres and uploads backups/PITR behind Pro, and the checkout confirmation displayed an immediate $20 charge; no purchase was made. Both volumes remain 5 GB in US East. Postgres retains the no-expiration 440 MB resize recovery point plus the 959 MB pre-security-patch backup; uploads has no backup or schedule.

Stage 1 shipped a disabled-by-default encrypted offsite path: a separately supervised worker uses a PostgreSQL advisory lock, PG18 custom dump, no-follow uploads archive, versioned checksummed manifest, pinned age encryption, immutable conditional R2 writes with HEAD verification, and coarse public-safe `PlatformSetting` monitoring. A standalone restore CLI rejects production/ambiguous/nonempty targets and validates ciphertext, manifest, Alembic head, representative counts, and upload extraction. The application has no R2 delete path and never receives the private age identity. `DEPLOYMENT.md` now documents configuration, key custody, R2 locks/lifecycle, preflight, restore, incident, rotation, and downgrade procedures.

Verification: Phase 98 suites **36 passed / 2 environment skips**; a separate real age v1.3.1 round trip passed with wrong-key rejection; full backend **3,034 passed / 20 skipped / 0 failed** (Phase 97 2,998 + 36). Python compile, shell syntax, diff check, dependency contract, and scoped secret scan passed. No migration; PG smoke not required. No frontend source change/build required.

Railway backend deployment `17d6ced9-bb75-46ab-84e8-d4973dfa8026` became Active from merge `ab4356f`. Build-time checksum/version gates passed, and the live container reports PostgreSQL client tools 18.6 and age v1.3.1. Production homepage/readiness/monitor passed; the monitor is overall `ok` with `offsite_backup.status=disabled` and no issue. Independent post-deploy monitor run `31815337987` succeeded and opened no incident.

Gate 2 is complete. Z explicitly approved and activated Cloudflare R2. The private Standard bucket `liquid-democracy-production-backups` is in ENAM with public access, public development URL, custom domain, CORS, and Data Catalog disabled. Prefix locks are enabled for `production/daily/` (7d), `production/weekly/` (35d), and `production/monthly/` (100d); lifecycle deletion is enabled after 8d, 36d, and 101d respectively. The production age identity was verified against password-manager and offline flash-drive copies; Railway holds only the public recipient, and temporary workstation/clipboard copies were removed. A single-bucket Object Read & Write credential and all 10 backup variables are live in Railway with `OFFSITE_BACKUP_ENABLED=false`; deployment `dabaa9dc-9987-43f7-b88e-ca4440634da0` is Active.

Stage 3's live preflight passed without dump/upload (`pg_dump`/`pg_restore` 18.6, age 1.3.1), and the monitor remains overall `ok` with `offsite_backup.status=disabled` and no offsite issue. A complete synthetic proof then used a short-lived isolated Railway PostgreSQL 18 service with one known database fixture and one 33-byte upload: one 194,792-byte ciphertext object was written under `test/phase98-synthetic-20260814/`, verified with the four allowed metadata fields, downloaded, checksum-verified, decrypted with a disposable test identity, and restored into a separate empty PG18 database. Alembic head `c5d6e7f8a9b0`, the known user count, and byte-for-byte upload content matched; zero decrypted temporary directories remained. Both disposable databases, all test key/files/credentials, and the temporary service were removed. The encrypted test object remains under an enabled one-day lifecycle rule; deletion observation is **NOT YET DUE** before August 15, 2026 at 12:45:50 EDT. Production prefixes remain empty and no production backup or production-data restore has run.

Stage 4 / Gate 4 is complete. Z activated Railway Pro and explicitly directed the team to proceed without waiting for the clock-bound R2 lifecycle observation. Railway now reports Pro active on August 14 for the August 5–September 5 cycle; billing history records a $10.73 subscription invoice on activation day, while ongoing plan economics are a $20 fee with $20 included usage. The keep/downgrade decision is due before September 5. Both `postgres-volume` and `user-uploads` have daily (6 days), weekly (1 month), and monthly (3 months) schedules enabled and independently re-verified as checked. New manual points are `2026-08-14 14:28` / 991 MB for PostgreSQL and `2026-08-14 14:30` / 150 MB for uploads; the older 959 MB and 440 MB PostgreSQL points remain. Railway currently reports backup storage at 6,284.48 minutely GB, $0.000003/GB/minute, and $0.0218 cycle-to-date. The four visible references total 2.54 GB, approximately $0.34 per 31 days if flat; scheduled retention will increase the footprint, subject to Railway's incremental storage behavior. Overall estimated workspace bill is currently $4.47. PITR remains off, no production Restore control was used, and the post-change monitor remains `ok` with no issues and offsite intentionally disabled. Pro-created backup accessibility after downgrade remains explicitly unproved.

Stage 5 is complete. A disposable Alpine 3.21 service and 5 GB volume were created in the production project/environment with unique Phase 98-only names and identifiers. Two synthetic files were read and hashed before backup. The first immediate 14:43 backup (790 MB referenced) restored the file entries as zero bytes despite valid pre-backup reads/hashes; no production resource was involved. After rewriting the originals, issuing an explicit `sync`, and allowing a settle interval, a second 14:46 backup (790 MB) was created. Both files were then mutated and flushed to distinct values/hashes. Railway staged a replacement volume, the exact non-production target was reviewed, and the staged test-only deployment restored the original marker, JSON nonce, and SHA-256 values exactly (`e9bef281…6b2c5` and `69d50cda…1afa5`). Native on-demand procedure now explicitly requires application quiesce/filesystem flush plus a settle interval before snapshot. Production Restore controls were never invoked.

Production backup plausibility was rechecked read-only: PostgreSQL remains 991/959/440 MB; uploads now has the 150 MB manual point plus a 150 MB daily scheduled point. Cleanup was applied after exact-ID review: the disposable service, original volume, and both replacement volumes were deleted, leaving no Phase 98 test resource while backend/PostgreSQL/frontend remained Online. Railway's displayed backup usage rose only from 6,284.48 to 6,299.19 minutely GB and $0.0218 to $0.0219 cycle-to-date; current usage stayed $2.04 and estimated bill $4.47.

Stage 6 encrypted-backup creation and verification are complete; the isolated restore is **NOT STARTED**. `OFFSITE_BACKUP_ENABLED=true` was the only staged Railway change and the replacement backend became Active. The worker completed a startup-driven daily backup at 19:03:31 UTC, then the required explicit `python -m offsite_backup_worker --once` run completed at 19:04:13 UTC in 11.227 seconds. The manual artifact is a private Standard `application/octet-stream` object under `production/daily/`, 58,761,416 encrypted bytes, format version 1, created at 19:04:01 UTC, with ciphertext SHA-256 `e22299dd2a0cf767e9afa60453454457c5cd049b1011237f7a56ac317f6caaf5`; its dashboard exposes exactly the four allowed custom metadata fields and no preview. Friday correctly produced only the daily class. The adjacent startup artifact is also 58.76 MB.

The object's daily prefix is covered by the enabled 7-day bucket lock and 8-day lifecycle deletion; weekly 35/36-day and monthly 100/101-day controls remain enabled. Bucket public access, public development URL, custom domain, CORS, and Data Catalog remain disabled. The monitor is overall `ok` with zero issues and records the manual success, no failure, zero consecutive failures, daily retention, and the exact ciphertext size. Fourteen to sixteen full retained copies project to 0.823–0.940 GB decimal at the current size, within R2's current 10 GB-month included allowance; expected present offsite storage remains $0/month, not a guarantee.

Stage 6's isolated production-data restore is complete. Z placed the identity only in `%TEMP%\phase98-age-identity.txt`; its one-line key shape was verified without displaying it. Temporary local recovery tooling reported PostgreSQL 18.6 and checksum-verified age v1.3.1. A new Cloudflare token was Object Read only, scoped solely to the backup bucket, and limited to 24 hours. A Railway service named `phase98-offsite-restore-disposable` ran PostgreSQL 18.6, and its distinct `phase98_restore_disposable` database was proven empty before restore. No production Restore control or production database target was used.

The standalone restore CLI downloaded and checksum-verified the manual 58,761,416-byte ciphertext, decrypted it locally, verified its format-1 bundle, member checksums, manifest, and Alembic head, then restored only into the disposable database. Independent aggregate checks found Alembic head `c5d6e7f8a9b0`, 50 public tables, 271 users, 13 organizations, 293 organization memberships, 153 proposals, 0 votes, and 197 delegations. Membership, proposal, vote, and delegation orphan counts were all zero. The signed manifest and extraction both reported zero upload files/bytes, so no private file was opened or displayed. No row content, email, ballot, token, or private organization text was printed.
...
## Phase 101 — Trusted High-Volume Proposal Creation ✅ Deployed (2026-08-23)

Spec: `phase101_high_volume_proposal_creation_spec.md`. Branch `phase-101/high-volume-proposal-creation`; implementation `c3c3249`; no-ff merge `f8b9a1b`.

The organization permission registry now includes `proposal.high_volume_create` with 32 enabled cells for Steward, 32 for Admin, 11 for Moderator, and 0 for Member. New and existing Steward/Admin roles receive the new cell enabled; Moderator/Member/custom roles receive no backfill. The reversible data-only migration preserves any pre-existing explicit cell and deletes only the new key on downgrade. Organization-scoped creation resolves the authenticated user's current effective target-org role after existing authorization/validation, then charges exactly one per-user bucket: ordinary 10/24h or trusted 10,000/24h. The global create route remains ordinary. Trusted successes annotate the existing `proposal.created` event with `high_volume_rate_tier: true`; rejected requests add neither a proposal nor an audit.

Multi-import remains sequential and resumable through the existing single-create endpoint. An ordinary selection above ten is blocked before any request with visible/associated guidance; trusted roles see the quiet enabled note. A shared API normalizer now preserves Pydantic detail precedence and accepts SlowAPI's safe top-level string `error`. Standard-tier and trusted safety-fuse 429s retain prior successes and explain that checked rows can be retried later.

Verification: Phase 101 backend **13 focused tests PASS** (11 limiter/route/scope/side-effect cases + 2 migration-cycle cases); production-like focused compatibility **214 passed / 0 failed**; authoritative full backend **3,065 passed / 20 skipped / 0 failed** (+14 over Phase 100's 3,051 baseline). The route-level scale fixture created and audited exactly 200 synthetic proposals locally. Frontend **30/30 PASS**; changed-file lint PASS; production build PASS at JS `index-DY8CvJfe.js` (1,873.78 kB / 476.80 kB gzip) and CSS `index-UmXqc-Uh.css` (66.16 kB / 12.40 kB gzip), with only the existing large-chunk warning. Python compile and `git diff --check` PASS. Reversible SQLite migration cycle PASS. PostgreSQL smoke PASS in fresh and upgrade-from-`c5d6e7f8a9b0` modes; both disposable containers were removed.

Production serves the exact Phase 101 bundle and all four load-bearing UI strings are present. The five proxy/service-worker smokes pass. Readiness is 200/`ok` with database connected; the monitor is 200/`ok` with zero issues and all components healthy. A non-mutating unauthenticated organization-create probe returns 401. Railway deployment-row lookup remains unavailable because the local CLI OAuth refresh is expired.

Interactive Role Permissions grant/revoke, 11-item disposable import, keyboard, and ~380px browser QA is **BLOCKED / not claimed** because the signed-in Chrome bridge still fails inside its trusted-runtime bootstrap before attaching to a tab. Source/unit/route/HTTP verification passes but does not replace that visual pass. No Massachusetts production proposal was created, deleted, archived, restored, or advanced during Phase 101 QA.

---

## Phase 102 — Scheduled Proposal Lifecycle and Bulk Controls ✅ Deployed (2026-08-25)

Spec: `phase102_scheduled_proposal_lifecycle_and_bulk_controls_spec.md`. Branch `phase-102/scheduled-proposal-lifecycle`; implementation commits `3204a7e` and `5de249f`; no-ff merge `f3d23fe`.

Added the durable nullable/indexed `Proposal.deliberation_end` clock and one route-independent lifecycle service for manual single, bulk, cosign, scheduled-worker, and reconciliation transitions. It owns timestamp capture, voting-end precedence, election-option locking, audit details, notification orchestration, and internal-only historical suppression. Draft creation/advance and seed/import paths now write coherent schedules. The bounded, lock-safe worker advances due ordinary deliberations and closes due budget-allocation/project votes with quorum-only pass/fail semantics; the Phase 102 gate leaves pre-existing non-budget close and cosign behavior active. Public-safe monitoring reports disabled, healthy, grace-window, and genuinely stuck states without identifiers.

Organization-scoped typed bulk endpoints now support retry-safe move-to-voting and schedule mutation with stable de-duplication, per-item savepoints/results, both required permission cells, active-vote shortening reasons, audit coverage, and a hard 500-ID request cap. Proposal Management is operation-first with eligibility-aware checkboxes, deterministic 500-row chunks, timezone confirmation, and one aggregate result. Proposal Detail uses the authoritative server deadline and distinguishes future, delayed, grace-window, and intentionally unscheduled states. No bulk withdraw/archive/delete/content-edit or client-selectable notification suppression was added.

Verification: **22/22 focused Phase 102 backend tests PASS**; all **3,107** backend cases were covered as **3,087 passed / 20 environment skips / 0 unresolved failures** (+22 passes over Phase 101). The matrix was partitioned because the local `.env` intentionally has `DEBUG=true`: 127 migration tests passed serially; 2,957 non-migration tests passed with 20 skips under four-worker work-stealing; the three limiter assertions bypassed by local debug all passed in a production-like `DEBUG=false` serial rerun. Frontend **34/34 PASS**; production build PASS at JS `index-DlZiYmVR.js` and CSS `index-D9QMkQJM.css`, with only the existing large-chunk warning. New Proposal Management/utility files pass targeted lint; Proposal Detail still reports its six pre-existing `set-state-in-effect` errors and one pre-existing dependency warning. Python compile, scoped secret scan, and diff check PASS. Reversible SQLite migration cycle PASS. PostgreSQL 16 disposable smoke PASS in fresh and upgrade-from-`d6e7f8a9b0c1` modes after the migration was made collision-safe; production reports Alembic head `e7f8a9b0c1d2` and the `deliberation_end` column present.

Production rollout followed the locked sequence. `PROPOSAL_SCHEDULE_AUTOMATION_ENABLED=false` was set before push. Backend deployment `cafa6b36-2634-4fc4-a60e-dbefae5e8d97` matched merge `f3d23fe` and succeeded; the disabled monitor reported zero overdue scheduled rows. The fresh 12:14:18 UTC inventory found Reform Table's expected **75 overdue ordinary**, zero overdue budget votes, and left all other history unchanged: `gamenights`, `just-fun-polls`, and `platform-feedback` each had one overdue ordinary; `calder-tool`, `demo`, and `demo-westgate-coalition` each had one invalid/missing-input row; `demo-cedar-hollow` had one cosign-gated and four invalid/missing-input rows.

The authorized 12:14–12:15 UTC Reform Table repair advanced **75/75** rows with zero failures, preserved each stored 500-day voting duration, produced exactly 75 reconciliation-trigger audits, and produced zero entered-voting notifications. Post-apply verification proved zero qualifying Reform Table overdue rows, the pre-existing voting and withdrawn rows/timestamps unchanged, no content/configuration mutation beyond lifecycle fields/`updated_at`, and unrelated overdue rows unchanged. The follow-up dry run at 12:15:21 UTC reproduced only the unrelated counts above.

The gate was then enabled and configuration deployment `f3b97a77-c04f-42d9-85ff-cd7f226239cc` succeeded. An exact-tagged disposable hidden organization outside Reform Table proved the live worker path: the proposal remained deliberating before its 12:19:36 UTC boundary, advanced at 12:21:07 UTC (within one five-minute interval), retained its own one-day voting duration, and created exactly one `deliberation_end_reached` audit plus one opted-in in-app voting-opened notification. Its worker-created snapshot, proposal, notification, audit, membership, role, user, and organization were then removed; an exact-tag readback returned absent. Final homepage health and readiness are 200, the five proxy/service-worker smokes pass, and the monitor is `ok` with zero issues, automation enabled, zero overdue scheduled rows, and a current decision-worker heartbeat.

Rendered Chrome QA remains **BLOCKED / not claimed**. The required Chrome skill failed before tab attachment because its trusted RPC resolved into the Browser plugin cache rather than the Chrome plugin trust path. The prescribed diagnostics found Chrome and the extension installed/enabled, but the native-host registry key `HKCU\\Software\\Google\\Chrome\\NativeMessagingHosts\\com.openai.codexextension` is missing. Per the skill, Codex did not repair it or substitute another browser tool; reinstalling the Browser plugin is required before desktop/~380px/keyboard rendered QA can be completed. Source contracts, frontend tests, live bundle activation, authenticated worker proof, unauthenticated bulk-route 401, and production HTTP checks pass, but do not replace that visual gate.

## Phase 101a — React Router Security and CI Hotfix ✅ Deployed (2026-08-25)

Spec: `phase101a_react_router_security_hotfix_spec.md`. Branch `phase-101a/react-router-security-hotfix`; implementation `e94de01`; no-ff merge `acca849`.

The dependency-only hotfix pins `react-router-dom` at `7.18.2` and resolves its single transitive `react-router` copy to the matching `7.18.2`, with no Router import migration or application-source change. The lockfile diff is limited to the root dependency plus those two package version/resolution/integrity entries. The production audit moved from two affected package nodes (one moderate direct finding and one high transitive finding aggregating five advisories) to zero vulnerabilities at every severity. Local clean install, exact graph inspection, **34/34 frontend tests**, production build, Router source-compatibility review, Python-independent diff check, and dependency-only scope review pass. The build produced JS `index-CzJ9iKPe.js` and CSS `index-D9QMkQJM.css`, with only the existing large-chunk warning.

GitHub Actions run [32899633349](https://github.com/Zachrelius/liquid-democracy/actions/runs/32899633349) is fully green for merge `acca849`: the Node 22 frontend job passed `npm ci`, the unchanged production-audit gate, all tests, and build; the backend job passed dependency audit and the full **3,107-case** suite. Railway frontend deployment `eddaa449-1d50-4cea-9ae7-75ff9363bdfc` is `SUCCESS` for the exact merge SHA. Production changed from `index-DlZiYmVR.js` to `index-CzJ9iKPe.js`; homepage and readiness are 200, database is connected, and the monitor is `ok` with zero issues.

The hotfix used a separate clean linked worktree based on the Phase 102 closeout, and the original checkout's staged/unstaged user files were restored byte-for-byte and to their original index state after merge. Rendered local/production routing QA is **BLOCKED / not claimed** because the standard Chrome bridge still fails before tab attachment: Chrome and the extension are installed/enabled, but `HKCU\\Software\\Google\\Chrome\\NativeMessagingHosts\\com.openai.codexextension` is absent. The Chrome skill forbids self-repair or a substitute browser path; reinstalling the Browser plugin remains required. Source inspection confirms `BrowserRouter`, route ordering, protected/admin guards, deep-link/query/sub-org APIs, and existing `react-router-dom` imports compile without unstable RSC/framework/SSR use, but that does not replace the visual matrix. No migration; PostgreSQL smoke not required. Recurring-cost delta: $0.

---

## Phase 102a — Graceful Verification Join Gate ✅ Deployed (2026-08-25)

Spec: `phase102a_graceful_verification_join_gate_spec.md`. Branch `phase-102a/graceful-verification-join-gate`; implementation `4eb2f3d`; no-ff merge `c034888`.

Membership-floor, minimum-age, and residency denials now retain their existing top-level structured contracts while adding one canonical public-only `membership_requirements` object. The shared helper uses the existing normalized residency reader and returns only floor, jurisdiction, residency policy, normalized allowed places, and minimum age; malformed settings fail to empty/null values and no applicant address, locality hash, identity, document, provider-session, attestation, or duplicate data can escape. Open join, approval request, and invitation acceptance remain non-mutating on denial, while an eligible verified resident still joins normally.

The public organization join flow now converts structured verification denials into an accessible modal instead of a raw 403 toast. It names the organization, explains the government-ID requirement, renders grammatical city/state/country and age rules with full state names, identifies Didit, links the provider privacy policy safely, and routes the primary action to `/settings#identity-verification`. Settings scrolls and focuses that stable anchor, preserves the server disclosure/provider-session sequence, offers Start for unverified/partial real accounts and a confirmed Update path for verified real accounts, and keeps demo stubs sealed. Unrelated errors retain the existing toast path; role, vote, delegate, invitation, and member-admin presentations were not converted to dialogs.

Verification: focused verification/join compatibility **167 passed / 0 failed**; complete frontend **42/42 passed**; changed-file lint, Python compile, production build, secret scan, and diff check passed. Local build output was JS `index-C2ZvG46Q.js` and CSS `index-IDIXqpbN.css`, with only the existing large-chunk warning. GitHub Actions run [32904303538](https://github.com/Zachrelius/liquid-democracy/actions/runs/32904303538) is fully green for merge `c034888`: frontend audit/tests/build passed and the authoritative backend suite completed at **3,096 passed / 19 environment skips / 0 failed** across 3,115 cases. No migration; PostgreSQL smoke was not required.

Railway frontend deployment `323583d2-73ab-4e9a-b5b9-99afb940dd29` and backend deployment `a5bd071b-5729-49aa-92d4-588a234b419b` are both `SUCCESS` for the exact merge SHA. Production serves `index-C2ZvG46Q.js`; homepage and bundle return 200, readiness is `ok` with the database connected, and the public monitor is `ok` with zero issues. The live bundle contains the dialog title, in-app CTA, Didit privacy-link text, Update verification control, and full `Massachusetts` copy. Public discovery confirms `ma-legislature` remains present with its two existing members; QA created no account, membership, audit, notification, or provider session and consumed no verification capacity.

Rendered desktop/~380px/keyboard/focus production QA is **BLOCKED / not claimed** because the standard Chrome bridge still cannot attach: Chrome and the extension are installed/enabled, but the native-host registry key is missing. Per the Chrome skill, no self-repair or substitute browser path was used. The historical tracked handoff document containing a plaintext Didit API key was not changed, rotated, or history-remediated; it remains an explicit separate security follow-up requiring Z authorization and a remediation plan.

---

## Phase 102b — Didit Terminal-Decision Enforcement and Verification Remediation ✅ Deployed (2026-08-30)

Spec: `phase102b_didit_terminal_decision_and_verification_remediation_spec.md`. Branch `phase-102b/didit-terminal-decision-fix`; implementation `42654d7`; no-ff merge `b1cf3a8`.

Didit promotion now fails closed unless the complete V3 webhook has both top-level and nested decision status `Approved`, declares ID verification, liveness, and face match, and supplies nonempty plural result arrays whose every node is approved. Singular V2, incomplete, abandoned, declined, transient, malformed, and contradictory payloads cannot write verification state or trigger completion, metering, duplicate detection, or purge. Non-approved updates preserve last-known-good user state, receive honest PII-free session/audit outcomes, and dedupe by normalized provider outcome so a later genuine approval remains processable. The read-only provider audit separately adapts Didit's documented flat session-retrieval response in memory without weakening the webhook envelope contract or retaining provider payloads.

The canonical organization PATCH now validates the merged proposal-policy state atomically: `always` requires a floor above email-only and a valid U.S. jurisdiction for address/residency floors. Org Settings blocks invalid saves, focuses and describes the missing field accessibly, exposes the residency-proof floor, and renders structured backend errors. A privacy-minimized cohort auditor and exact-target remediation tool enforce live-provider and database compare-and-set guards, row locks, fresh provider readback, preservation of append-only history, suspension rather than deletion of gated memberships, and idempotent replay.

Local verification: Phase 102b remediation **20/20 PASS** after final review; affected Didit compatibility **94/94 PASS**; authoritative full backend **3,114 passed / 20 environment skips / 0 failed** across 3,134 cases. Frontend **45/45 PASS** and production build PASS at JS `index-L_xSBRWz.js` (1,887.22 kB / 481.18 kB gzip), with only the existing large-chunk warning. New utility/test lint PASS; `OrgSettings.jsx` still reports its three pre-existing lint findings (`set-state-in-effect` twice and one unused catch variable), with no Phase 102b lint finding. Python compile, scoped secret scan, and `git diff --check` PASS. GitHub Actions run `33312617469` is fully green for merge `b1cf3a8`: dependency audits, frontend tests/build, and the complete backend suite all passed. No migration; PostgreSQL smoke was not required.

Production rollout followed the locked backend-first sequence. Readiness and the public monitor were green before deployment; the encrypted offsite backup had succeeded at 11:00 UTC with zero failures, and backup preflight confirmed PostgreSQL 18.6 clients plus age 1.3.1. Backend-first deployment `f6f19698-2e43-4cea-b427-89464e6e34eb` succeeded before any audit or correction. The dry-run cohort audit found **1 confirmed valid, 1 confirmed false positive, 0 provider-unavailable, 0 history mismatch, and 1 non-applicable** without emitting full identifiers or identity payloads.

The `ma-legislature` gate was repaired through the live validated organization PATCH from invalid `always / null / null` to `always / address_on_id / MA`. The exact remediation preview matched `address_on_id / didit / approved_purge_failed / provider abandoned`, then the locked apply re-read the provider and corrected exactly one user and one session, suspended exactly one affected membership, preserved two historical votes and one consumption row, retained the original completion audit, and added one corrective verification audit plus one membership-suspension audit. Post-check proved all false-derived identity fields cleared, corrective timestamp present, provider still `Abandoned`, session marked `remediated_false_positive`, and replay idempotent with zero mutations. Live read-only enforcement proved the remediated account receives the structured MA vote denial and is absent from canonical eligibility, while an active legitimately verified MA member passes the same floor and is eligible; no production ballot was manufactured for QA.

Merge-triggered backend deployment `c90fbd64-41d5-4b43-b3c3-a377877641f9` and frontend deployment `996c4626-9b90-46c5-be5d-87c898e86ecc` are `SUCCESS`. Production serves `index-D3hK9M5U.js`, which contains the new policy validation copy. Readiness is `ok` with database connected; the monitor is `ok` with zero issues; and all five proxy/service-worker smokes pass.

Rendered Org Settings and affected-account desktop/~380px/keyboard QA remains **BLOCKED / not claimed**. The Chrome bridge still fails before tab attachment at its trusted native-host boundary; per the Chrome skill and AGENTS convention, no self-repair or substitute browser was used. Source tests, live bundle inspection, authenticated production state checks, eligibility evaluation, and HTTP smoke pass but do not replace that rendered gate. No real-person verification session was created and no identity payload, full provider session identifier, document data, address, DOB, image, selfie, or biometric was printed or persisted by Phase 102b tooling.

---

## Phase 103 — Proposal Feed Scaling and Overload Resilience ✅ Deployed (2026-08-30)

Spec: `phase103_proposal_feed_scaling_and_overload_resilience_spec.md`. Branch `phase-103/proposal-feed-scaling`; implementation commits `d98c709`, `00521e2`, and `497d3a0`; local-proof/documentation commit `de40345`; no-ff merge `ecfed31`.

The proposal-list incident was application-side request amplification rather than a Railway platform failure. For 225 active proposals, the old member page issued one list request plus 225 result requests plus 225 my-vote requests: exactly **451 proposal-data calls**, or up to **453 page-component calls** after topics and sub-organization metadata. The member page now makes exactly three calls after organization context (compact feed, topics, and sub-organization metadata), makes zero list-page `/results` or `/my-vote` calls, and adds exactly one feed request per Load more. Public landing previews request five compact rows, and the full public feed is bounded and cursor-paginated.

Three compact cursor endpoints now apply status, topic, membership, private-sub-organization, and delegated-vote semantics before pagination. The canonical batch viewer resolver preserves direct vote, topic precedence, organization-wide fallback, two-hop delegation, cycle handling, mixed ballots, and weighted eligibility without running the tally engine per card. Malformed cursors fail safely. Compact cards preserve title, status, voting method, topics, author, date, sub-organization, deadline/stable/count context, and the viewer's vote; live aggregate bars and winner computation moved off list cards and remain authoritative on proposal detail.

Overload behavior now uses a five-second database checkout timeout and a narrow `database_busy` JSON 503 with request ID, `Retry-After: 2`, and `no-store`. Liveness is async and database-independent; readiness is isolated and bounded. The public monitor reports safe pool capacity/occupancy plus rolling timeout counts. The shared frontend response parser handles JSON and non-JSON responses across normal, refresh, form, download, and login paths; HTML 502/503/504 responses produce **“The service is temporarily busy. Please try again in a moment.”**, network failures remain distinct, bounded GET/auth timeouts cancel correctly, and aborts are silent.

Measured route SQL counts were member **18** (budget 30), public **5** (budget 12), global **13** (budget 30), and first-page `unvoted` over 250 voting proposals **25** (budget 40). The required second PostgreSQL 16 load run completed **158/158 HTTP 200**, zero 503/unexpected 5xx, feed p50/p95/p99 **492.84/1,168.47/1,189.03 ms**, health p95 **225.32 ms**, and maximum payload **20,746 bytes**. Cursor traversal was 266 expected/unique with zero duplicate, missing, or unexpected rows. Pool occupancy was 1/5 baseline, 5/5 peak, and 1/5 five seconds later, with zero timeouts, zero idle-in-transaction sessions over five seconds, and zero waiting locks. The first run's health p95 was 378.15 ms and missed the 250 ms gate; the mandated rerun passed rather than waiving it.

The 2,500-row EXPLAIN fixture measured default/voting/topic/cursor execution at **2.905/2.748/5.492/2.764 ms** using existing indexes and bounded in-memory top-N sorts with zero shared reads. No evidence justified an index or migration; PostgreSQL migration smoke was therefore not required. No Railway pool-size or environment-variable change was made.

Verification: authoritative backend **3,131 passed / 20 environment skips / 0 failed** across 3,151 cases, up 17 passes/cases from Phase 102b's 3,114/20 baseline; focused compatibility **68/68 PASS**. Frontend **62/62 PASS**, changed-file ESLint clean, production build PASS at JS `index-BvlqA3hN.js` (1,889.02 kB / 481.93 kB gzip) and CSS `index-B6zp5ENh.css` (66.59 kB / 12.46 kB gzip), with only the existing large-chunk warning. Python compile, route budgets, PostgreSQL load/EXPLAIN proof, and diff checks passed. GitHub Actions run `33323266143` is fully green for merge `ecfed31`: dependency audits, frontend tests/build, and the complete backend suite passed; its frontend job completed in 17 seconds and backend job in 23m36s.

Railway backend deployment `bc8a19f7-2eab-4ef9-b5eb-d2fdcd97eed6` and frontend deployment `cbf3c86a-92c4-42e0-8e9e-2d1ab2332002` are `SUCCESS` for merge `ecfed31`. Production serves `index-BvlqA3hN.js`. Homepage, health, readiness, and monitor return 200; readiness reports the database connected, and the monitor is `ok` with zero issues, pool capacity 5, one checkout, zero overflow, a five-second timeout, and zero timeouts in 15 minutes. Anonymous two-page cursor verification returned 5+5 unique rows with nullable viewer votes; the logged-out member endpoint returned the expected 401. No production mutation or load was performed.

Rendered desktop/~380px/keyboard/focus/filter/error-state QA is **BLOCKED / not claimed**. The mandated Chrome client failed before creating its agent because `browser-service.mjs` did not resolve within a configured trusted code path, which also made the prescribed in-client troubleshooting document unreachable. Per the Chrome skill and `AGENTS.md`, no substitute browser was used. The exact bundle, source tests, build, and production HTTP/cursor behavior pass but do not replace the rendered matrix.

New debt is bounded to the unchanged raw-array consumers: Proposal Management, SubOrgProposals, SubOrgSettings, Polis pickers, and other admin/secondary surfaces. The recommended admin and secondary proposal-list pagination pass is **NOT STARTED**; tally snapshots, replica readiness, and any isolated Railway load pass also remain explicitly unstarted.

---

## Phase 104 — Admin and Secondary Proposal Pagination ✅ Deployed (2026-08-30)

Spec: `phase104_admin_and_secondary_proposal_pagination_spec.md`. Branch `phase-104/admin-secondary-proposal-pagination`; implementation commits `78a02be` and `db1e1ff`; local-proof documentation `e4de14c`; initial no-ff merge `ffc1096`; production-smoke hotfix `b52f8f1`; final hotfix merge `c138c99`.

All known internal frontend consumers of the legacy full-proposal arrays now use bounded purpose-built feeds. Proposal Management adds server-side status/scope/title/eligibility filters, append pagination, cross-page organization-keyed selection metadata, loaded-only select-all wording, explicit confirmation before filter/operation changes clear selection, and partial/network retry reconciliation that removes only completed rows. Sub-organization proposal administration uses server scope and bounded reloads; deletion settings use one exact impact endpoint whose proposal/topic predicate is shared with transactional DELETE, includes archived proposals, and keeps the conservative 409 behavior. Polis voter/admin pages share an independent exact reverse-link feed. Structural bulk eligibility is canonical across listing and action preconditions, including the corrected cosign-gated `set_end` eligibility.

The global, organization, and public legacy arrays remain compatibility-only: deprecated in OpenAPI, default 25, maximum 50, stably offset-paginated, and accompanied by deprecation/next-page headers. Internal source audit finds zero calls to them. A production smoke caught their first `Link` implementation reflecting Railway's internal proxy authority; hotfix `b52f8f1` now emits an origin-relative URI, preserves filters and repeated query pairs, and cannot reflect the request host.

Before Phase 104, an isolated 250-proposal raw array returned 858,401 bytes in 344.62 ms and executed 759 SQLite statements. New route-level counts are management 7, all-filter management 8, deletion impact 6, and Polis links 8. The required monitor-enabled PostgreSQL 16 rerun completed 238/238 measured requests at HTTP 200 with zero unexpected 5xx or pool-timeout 503s. Management p50/p95/p99 was 106.93/299.72/325.17 ms, health p95 85.38 ms, and the largest management page 18,391 bytes. Pool occupancy was 1/5 baseline, 5/5 peak, and 1/5 within five seconds; waiting locks and sessions idle in transaction over five seconds were zero. Exact management/sub-organization/Polis traversals returned 250/50/63 rows with no duplicate, missing, unexpected, or lookalike-ID matches. The first run's measured application requests also all returned 200, but its separate monitor sampler returned 503 because monitoring was disabled in that proof process and the synthetic administrator used an invalid alert address; the corrected rerun returned monitor 200 before, during, and after load.

At 2,500 proposals, EXPLAIN execution was 1.980 ms default management, 0.850 ms filtered, 2.043 ms next page, and 1.186 ms exact Polis links. Existing selective indexes and bounded in-memory sorts were sufficient, with zero shared reads. No index, normalized link table, migration, pool increase, or Railway environment change was justified; PostgreSQL migration smoke was not required.

Verification: authoritative backend **3,140 passed / 20 environment skips / 0 failed** (+9 passes from Phase 103), focused Phase 104 **9/9**, and affected compatibility **174/174**. Frontend **69/69 PASS** (+7 from Phase 103), changed-file ESLint clean, and production build PASS at JS `index-DIzEE_fF.js` (1,894.98 kB / 483.68 kB gzip) and CSS `index-CIRcFmI8.css` (66.74 kB / 12.48 kB gzip), with only the existing large-chunk warning. Python compile, secret scan, diff checks, SQL budgets, PostgreSQL load, and EXPLAIN proof passed.

Railway frontend deployment `024379e4-dea8-423d-bdc0-1803fcb2c6a1` serves production bundle `index-B7Px6sck.js`. Final backend deployment `4416ee1a-57d3-41dc-8596-0c20da38bb6f` is successful for hotfix merge `c138c99`; watched paths correctly skipped an unnecessary frontend rebuild for that backend-only merge. Production homepage, health, readiness, and monitor return 200; readiness reports database connected, monitor reports `ok`/zero issues, rolling 5xx and pool timeouts are zero, and pool occupancy is 1/5. The compact public feed returns five rows with a cursor. The legacy public route returns exactly 50 rows with deprecation headers and the relative next link `</api/orgs/ma-legislature/public/proposals?limit=50&offset=50>; rel="next"`. No production mutation or load was performed.

Rendered desktop/~380px/keyboard/filter/selection/sub-organization/Polis QA remains **BLOCKED / not claimed**. The mandated Chrome runtime fails before browser binding because `browser-service.mjs` does not resolve within a configured trusted code path; `agent` is never initialized, so the prescribed troubleshooting document is also unreachable. No substitute browser was used. Versioned tally summaries, broader collection pagination, sub-organization content relocation, replica readiness, and Chrome tooling repair remain **NOT STARTED**.

---

## Phase 105 — Organization Branding and Verification Clarity ✅ Deployed (2026-08-30)

Spec: `phase105_org_branding_and_verification_clarity_spec.md`. Branch `phase-105/org-branding-verification-clarity`; frontend commits `5d39bc3` and `37e1fca`; backend commit `9ebe91b`; spec commit `80db7c4`; no-ff merge `8a95f8f`.

Clusters B–F and G are **DONE**. Organization branding now supports a nullable `header_text_color` with `#RGB`/`#RRGGBB` validation, internal/public/explore/sub-organization serialization and inheritance, audit diffs, reset behavior, exact header CSS variables, and a desktop/mobile navigation, organization switcher, and notification-badge color sweep. The settings preview shows an advisory WCAG contrast warning without blocking a save, while an unconfigured organization retains the prior visual defaults.

Membership, role, organization-proposal, and author-proposal controls now expose only **none**, **identity verified**, and **verified resident** choices as applicable; no new address-only choice or separate residency checkbox remains. Resident always means `address_on_id` evidence plus a match against the organization's shared normalized residency scope. The organization proposal selector coherently offers never, author chooses, always identity, or always resident. Existing address-only, dead-rung, jurisdiction, and missing-flag combinations remain enforceable and appear as disabled legacy placeholders until an administrator deliberately selects a supported replacement; unrelated partial writes preserve them. This includes the legacy Massachusetts `always / address_on_id / MA` behavior repaired in Phase 102b. A signed, one-hour, organization-scoped compatibility token preserves an imported legacy proposal gate without reopening address-only writes to ordinary API clients.

Migration `f8a9b0c1d2e3` adds nullable `Proposal.verification_require_residency` above prior head `e7f8a9b0c1d2`. The typed effective-requirement resolver is wired through both proposal-create paths, draft/update and response builders, compact feeds, voting/eligibility, elections, templates, imports, demo/direct seeds, and inheritance. SQLite upgrade/downgrade/upgrade and the required PostgreSQL 16 fresh plus prior-head upgrade smoke both passed; the disposable containers were removed. Production schema-head lookup was unavailable through the environment's unauthorized Railway/private-database path, so it is not independently claimed from a direct database query.

Account Settings now edits the account default or a sorted top-level organization override from the existing bounded organization hydration call, with typed set/reset responses, effective and stored values on authenticated `OrgOut`, the stable `/settings?displayNameOrg={slug}#display-names` deep link, invalid-slug normalization, and abort/generation checks that suppress stale results. Public organization, Explore, feed, implicit-sub-organization, and other-member responses do not receive these current-user fields. The 30-membership global-name preflight stays within **10 SQL statements** and skips organizations whose explicit override makes the global fallback irrelevant.

Legal-name display policy now explicitly scopes to **public delegates** or legacy-compatible **all verified members**. The canonical tenant-scoped gate covers both `public` and `public_accepting`, requires genuine identity evidence and usable legal-name components for the strict public-delegate rule, applies stronger membership/residency and duplicate-state gates, blocks noncompliant visibility transitions and per-organization/global name changes, and validates existing public profiles atomically before policy activation. Typed errors and bounded activation conflicts contain only safe IDs, display names, scope/mode, and reason codes. Privacy/source audit found no legal name, address, DOB, verification hash, provider payload, document, selfie, or biometric value in public responses, structured errors, or new audits.

Cluster A's source/route inventory is **DONE**, including the prior global-only display-name writer and every public/public-accepting transition. The requested aggregate production settings/profile inventory is **BLOCKED**: Railway CLI was unauthorized and the environment explicitly denied the otherwise spec-authorized read-only private-database query, so no retry or workaround was attempted and no aggregate counts are claimed. Source and production-shaped fixture coverage confirms the supported shapes and their handling: absent name-match scope with a non-off legacy rule remains `all_verified_members`; touched/new resident choices require a valid shared country/state/city scope; untouched address-only, jurisdiction, stale-ladder, and missing-residency-flag states remain unchanged; and invalid scope rows are discarded without discarding an otherwise valid country entry.

Verification: authoritative backend **3,172 passed / 20 environment skips / 0 failed** (+32 passes from Phase 104), focused Phase 105 plus migration **32/32**, affected compatibility **247 passed / 17 expected skips**, and post-triage expansion **111/111**. Frontend **77/77 PASS** (+8 from Phase 104), changed-file ESLint clean, and production build PASS at JS `index-B_UqZseD.js` (1,907.54 kB / 486.78 kB gzip) and CSS `index-Do5s37dd.css` (67.20 kB / 12.56 kB gzip), with only the existing large-chunk warning. Python compile, diff checks, migration reversibility, PostgreSQL smoke, privacy audit, and query-bound proof passed. GitHub Actions run [33337403985](https://github.com/Zachrelius/liquid-democracy/actions/runs/33337403985) is fully green for exact merge `8a95f8f`: frontend audit/tests/build completed in 22 seconds and backend audit/full tests in 24m01s. The first local non-migration matrix found five expected contract-assertion drifts and one real Canadian-country normalization defect; the defect was fixed to retain a valid country while dropping its unusable city, the exact six and expanded regressions passed, and the authoritative full rerun was clean.

Railway backend deployment `9b25eddf-b098-4f3b-ab70-063ed4a811bb` and frontend deployment `d297b96b-f4f0-49e5-87f0-564dfad342be` are `SUCCESS` for merge `8a95f8f`. Production serves `index-DKjP7ryU.js`. Homepage, health, readiness, and monitor return 200; readiness reports database connected, monitor is `ok` with zero issues, rolling 5xx and pool timeouts are zero, and pool occupancy is 1/5. Public branding returns the new nullable header field, directory discovery returns five organizations, and a bounded public proposal feed returns five rows with cursor metadata and no sensitive identity fields. No production organization policy, user name, identity state/payload, paid verification capacity, Railway environment/configuration, or production data was mutated, and no production load was run.

Rendered authenticated desktop/mobile/keyboard settings, name-editor, branding-preview, and delegate-transition QA is **BLOCKED / not claimed**. The mandatory Chrome runtime failed before browser binding because `browser-service.mjs` did not resolve within a configured trusted code path; `agent` was never initialized, so its prescribed troubleshooting document was unreachable. Per the Chrome skill and `AGENTS.md`, no substitute browser, auth-token workaround, or repeat attempt was used. Chrome trusted-path repair and the environment-approved aggregate production inventory remain explicit follow-ups **NOT STARTED**.

---

## Phase 107 — Live Connection Security and Credential Hygiene — Deployed (2026-09-04)

Spec: `phase107_connection_security_spec.md`. Branch `phase-107/connection-session-security`; code commits `892d6bf` and `85ee4f7`; no-ff release merge `e3ecd62`. Full evidence: `docs/phase107_closeout.md`; independent review: `docs/phase107_review.md`.

Live tally subscriptions now release database connections before socket waits/sends, revalidate current proposal access and active-account/token status before every update and periodically while idle, enforce expiry, bound transport operations/fan-out, and clean up subscriptions on failure/cancellation. Vote/retract release their committed-work read transactions before broadcasts. The original route was independently reproduced retaining the only pooled connection and blocking an ordinary query; the corrected actual route supports four sockets with zero idle checkouts and real vote/retract traffic against that same one-slot pool.

Added 24 backend regression cases. Authoritative Linux CI run `33929594302` passed **3,197 tests / 19 environment skips / zero failures**, plus frontend audit/tests/build and six credential-guard tests. The 3,216-case local matrix initially had two monitoring assertions affected by deliberately disabled workers; the full 15-test monitoring file passed with intended configuration restored, yielding 3,196 local passes / 20 skips / no unresolved failures. Focused security/voting/visibility matrix: 142 passed. Compile, source guard and whitespace checks passed. No migration; PostgreSQL migration smoke not required.

The historical Didit key was removed from current source, and a narrowly scoped tested CI guard catches literal Didit secret assignments without printing values. Read-only comparison confirmed Railway's currently configured backend key differs from the old value, supporting Z's recollection of prior replacement. Provider-side revocation of the old key remains independently **UNVERIFIED**; no key was displayed, exercised or rotated, and history was not rewritten. Provider metadata confirmation is **NOT STARTED**; procedure and exact follow-up dispatch are in the credential remediation document and closeout.

Railway backend deployment `7a645ef4-807c-4004-843a-a3a92dc425d4` is **SUCCESS** for exact merge `e3ecd6284077984de89d386588dd45fc407728f4`. Production homepage, liveness and readiness pass; monitor is `ok` with zero issues. A single unauthorized socket closes 4401 without tally data, and readiness remains healthy afterward. Frontend correctly remains on `index-DKjP7ryU.js` because this pass changes no frontend source. Required Chrome MCP is unavailable; rendered browser QA is not claimed. The original dirty planning checkout is preserved, including its older local master state. No paid capacity, production data cleanup or infrastructure/secret configuration change was introduced.

---

## Phase 108 — Demo Email Suppression — Deployed (2026-09-15)

Spec: `phase108_demo_email_suppression_spec.md`. Branch: `phase-108/demo-email-suppression`; spec `155001a`; implementation `6a683a9`; no-ff release merge `224e553`. Evidence: `docs/phase108_closeout.md`, `docs/phase108_review.md`, and `docs/resend_quota_investigation_2026-09-15.md`.

The quota investigation found 85 provider-accepted submissions on September 14 (32 daily + 53 weekly digests), all to fictional `demo.example` addresses, immediately before the Resend 80%-quota warning. The shared email sender now handles that exact parsed mailbox domain before Resend, SMTP, console output, or delivery monitoring. It supports case/whitespace/display-name/terminal-dot variants, logs suppression without content, and returns handled=True without clearing real delivery-failure streaks. Real users, including people participating in demo organizations, retain their existing email behavior. No user records, preferences, demo reset content, schema, provider settings, or frontend behavior changed.
