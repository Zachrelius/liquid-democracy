...
/**
 * BinaryVoteFlowGraph — original binary-only force layout extracted unchanged
 * from the pre-Phase-7B VoteFlowGraph. Green/red half-plane zones, abstain
...
export default function BinaryVoteFlowGraph({ data, onNodeClick }) {
  ...
  useEffect(() => {
    ...
    const splitGap = 2;
    const bottomZoneY = height * 0.78;

    g.append('rect')
      .attr('x', -BIG)
      .attr('y', -BIG)
      .attr('width', centerX - splitGap + BIG)
      .attr('height', bottomZoneY + BIG)
    ...
    g.append('rect')
      .attr('x', centerX + splitGap)
      .attr('y', -BIG)
      .attr('width', BIG + centerX)
      .attr('height', bottomZoneY + BIG)
      .attr('fill', ZONE_COLORS.no);
    g.append('text')
      .attr('x', centerX + centerX * 0.5)
      .attr('y', 24)
      .attr('text-anchor', 'middle')
    ...
    const simulation = d3
      ...
      .force(
        'charge',
        d3.forceManyBody().strength((d) => (d.type === 'non_voter' ? -40 : -120))
      )
      .force(
        'link',
        d3.forceLink(validEdges).id((d) => d.id).distance(60).strength(0.3)
      )
      .force(
        'x',
        d3
        ...
        <div className="absolute bottom-2 left-2 right-2 bg-white border border-gray-200 rounded-xl shadow-lg p-4 text-sm z-10 md:left-auto md:right-2 md:w-72 md:bottom-2">
          ...
          <div className="space-y-1.5 text-gray-600">
            ...
            {selectedNode.delegator_count > 0 && (
              <p>
                {selectedNode.delegator_count} user{selectedNode.delegator_count > 1 ? 's' : ''} delegate
                to this person (total weight: {selectedNode.total_vote_weight})
              </p>
            )}
