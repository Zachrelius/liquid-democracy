...
- **Sub-numbered passes (Phase 40a+):** `phaseXX<letter>_<short-name>_spec.md` — e.g. `phase40a_trap_fix_hotfix_spec.md`. The letter suffix sorts AFTER the main `phaseXX_*` files in directory listings (since `'a' > '_'` in ASCII), so sub-numbered passes group cleanly under their parent. Multiple sub-passes use successive letters (`40a`, `40b`, `40c`). Pre-Phase-40a specs continue to use the legacy underscore-number convention (`phase18_5_*`, `phase23_1_*`, etc.) — those are grandfathered to avoid renaming churn through PROGRESS.md + code comments + test file names. Dots in filenames are forbidden in both conventions; they break tab-completion + grep.
...
**Chat dispatch is one line and conveys go-ahead.** The dispatched prompt (what Z pastes to the Code team) is shaped like `Read and execute phaseXX_<short-name>_spec.md` — names the spec AND grants the go-ahead in one sentence so the team doesn't pause for approval after reading. "Read" alone is not enough; "execute" (or "read and ship," "read and proceed," etc.) is the load-bearing word that closes the no-go-back-to-Z loop. Every load-bearing fact for the team lives in the spec, which is checked in, searchable, and the single source of truth. The planning agent's chat message to Z can include a summary, any actions Z should take, and team-choice rationale for Z's benefit; those live in the planning agent's message to Z, NOT inside the dispatch prompt itself. Rationale: single source of truth + system logging + forces spec self-sufficiency + nothing for the dispatch prompt to drift from. The chat dispatch is the trigger, not the content.
...
**Verify backend deploy success after push.** Standard operational hygiene — do not assume `git push` triggered a successful deploy. Check both (a) Railway dashboard / `railway deployment list --service backend` shows a deployment row matching the pushed commit AND (b) a backend smoke endpoint returns the expected response. Frontend service auto-deploys reliably; backend was unreliable across Phases 32 / 32.1 / 32.2 but has held clean for 5 consecutive passes (33 → 34 → 34.1 → 34.1 hotfix → 35). If the pattern breaks again, fall back to manual trigger: `railway up --service backend` (uploads current source directly; works even when push-trigger is stuck), empty commit push, or dashboard "Redeploy" button.
...
**Response builders that call org-aware resolvers MUST take `db: Session` as required (Phase 32.2 hotfix #1 + Phase 33 B-cluster lesson).** When a response builder (e.g., `_build_proposal_out`) calls a resolver that reads `org.settings` (e.g., the 4-mode write-ins / pre-voting / visibility resolvers), the `db: Session` argument is a REQUIRED positional parameter — never `Optional[Session] = None`. Three passes in a row hit the same bug: builder accepted optional db, most call sites passed nothing, builder couldn't load the Organization row, resolver silently fell back to platform defaults, prod misbehaved while tests stayed green. Phase 33 B1 closes the resolver-layer half with a loud-failure guard (`_assert_org_loaded_for`) that raises when proposal.org_id is set but org=None — but the builder-layer convention is still load-bearing: builders that don't need db stay unchanged; builders that call org-aware resolvers must require it. Catch in code review.

**New `Organization` field the FE depends on MUST be surfaced on `OrgOut` + asserted in the serializer-coverage test (Phase 45a + Phase 46 lesson, codified in Phase 46a).** Phase 45a hotfix #1 (OWNER_ONLY_KEYS missing from `user_permissions`) and Phase 46 hotfix #1 (`proposal_creation_mode` missing from `OrgOut`) shipped the identical model-vs-response gap: a new `Organization` column was added, all backend tests passed, the FE silently broke in prod because the field never reached `currentOrg`. Phase 46a `backend/tests/test_phase_46a_orgout_serializer_coverage.py` closes this with an explicit must-surface allow-list — adding a new FE-facing field is a three-line change in the same pass:
  1. Add the field to `schemas.OrgOut` with a sensible default.
  2. Populate it in `routes.organizations._org_to_out`.
  3. Append the field key to `_MUST_SURFACE_FIELDS` in the coverage test.

The check is an explicit allow-list (not "every column on `Organization`") because some columns are intentionally internal (worker bookkeeping, demo flags). The convention is: if the FE reads it, it's on the list.
