...
| Browser verification (Chrome MCP, prod) | Yes | **TBD** — recommend dispatching the QA sub-agent for: (1) set a term on the Steward title via the new "Term…" button → confirm next-election date surfaces; (2) toggle 'Scheduled / fixed-term' in OrgSettings; (3) confirm OrgTitlesPanel reflects the term + next-due. Routine UI surfaces (the new button + display row) PASS-by-source given the FE build succeeded + the underlying API contract is covered by the 14 Phase 49 + 24 Phase 47 tests. |
