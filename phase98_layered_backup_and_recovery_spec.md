...
### Stage 6 — PARTIAL, ENCRYPTED BACKUP VERIFIED; RESTORE NOT STARTED
...
- No ciphertext was downloaded, no private identity was retrieved, and no restore command or production Restore control was used. Execution is intentionally paused immediately before the isolated download/decrypt/restore rehearsal. Z should now place the production identity in a temporary local plaintext file outside the repository and report only that the file is ready; never paste the key into chat or store it in Railway, R2, GitHub, repository files, logs, email, or Google Drive. The temporary operator copy will be removed after disposable restore evidence and cleanup; the password-manager and offline originals remain.
...
- **Stage 6 — PARTIAL.** Production encrypted-backup creation, R2 metadata verification, prefix-lock/lifecycle coverage, and monitor proof passed. The isolated production-data restore and post-restore cleanup/normal-run proof have not started and require the temporary private-key file checkpoint above.
