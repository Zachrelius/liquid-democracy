...
**Status:** Stages 0-2 and 4-6 are complete. Stage 3's live R2 preflight and synthetic backup/restore proof passed; its one-day lifecycle deletion observation remains clock-bound and Z explicitly directed later stages to proceed without waiting for it. Railway Pro native coverage, disposable native restore mechanics, production encrypted-backup creation, and the isolated offsite production-data restore are proven without restoring over any production resource. Stage 7 downgrade continuity is not started.
...
- Deployed the single intended Railway change, `OFFSITE_BACKUP_ENABLED=true`. The replacement backend became Active and production health stayed green. The separately supervised worker completed a startup-driven backup at 19:03:31 UTC, then the required explicit `python -m offsite_backup_worker --once` run completed at 19:04:13 UTC in 11.227 seconds. Both runs created private Standard ciphertext objects under the dated `production/daily/` prefix; Friday classified the run only as `daily`.
- Verified the explicit-run object without downloading it. Its basename is `liquid-democracy-20260814T190401Z-e22299dd2a0c.tar.age`, content type is `application/octet-stream`, size is 58,761,416 encrypted bytes, creation metadata is `2026-08-14T19:04:01Z`, artifact format is `1`, and ciphertext SHA-256 is `e22299dd2a0cf767e9afa60453454457c5cd049b1011237f7a56ac317f6caaf5`. The object exposes only the four allowed custom metadata fields and has no preview. The adjacent startup-driven ciphertext is also listed at 58.76 MB.
- Re-verified bucket privacy plus enabled prefix controls. Because the object key begins `production/daily/`, the enabled 7-day bucket-lock rule and enabled 8-day lifecycle-deletion rule apply. Weekly/monthly objects were correctly absent for this Friday run; their 35/36-day and 100/101-day lock/lifecycle rules remain enabled. Public access, public development URL, custom domain, CORS, and R2 Data Catalog remain disabled.
- The public production monitor is overall `ok` with zero issues. Its offsite component reports `status=ok`, the explicit 19:04:13 UTC success, no failure, zero consecutive failures, 58,761,416 encrypted bytes, and retention class `daily`.
- At the current artifact size, fourteen to sixteen retained full ciphertexts project to 822,659,824–940,182,656 bytes (0.823–0.940 GB decimal) before the small manifest/compression variation between runs. This remains within R2 Standard's current 10 GB-month included allowance, so expected present offsite storage cost remains $0/month, not a guarantee. The extra first-day manual proof adds one temporary 58.76 MB daily object until lifecycle expiry.
- Z created the temporary production identity only at `%TEMP%\phase98-age-identity.txt`, outside the repository. The operator verified a one-line age secret-key shape without displaying its contents. Temporary recovery tooling used official PostgreSQL 18.6 Windows binaries and age v1.3.1; the age archive matched the digest published in the official GitHub release metadata.
- Created a separate Cloudflare account API token named `Phase 98 isolated restore read-only 20260814` with **Object Read only** permission, scoped only to the production backup bucket, and a 24-hour TTL. Its one-time values were written only to Windows TEMP. The token was revoked immediately after restore; the production bucket-scoped writer remains present.
- Created a Railway PostgreSQL 18.6 service named `phase98-offsite-restore-disposable` with a distinct disposable database named `phase98_restore_disposable`. The target was verified to contain zero user tables before restore. Its host and database name differed from the explicit production safety inputs; no production restore control or production database target was used.
- The standalone restore CLI downloaded the verified 58,761,416-byte manual ciphertext, matched SHA-256 `e22299dd2a0cf767e9afa60453454457c5cd049b1011237f7a56ac317f6caaf5`, decrypted locally with the temporary identity, validated the format-1 bundle/member checksums and Alembic head, and restored only into the empty disposable target.
- Independent aggregate validation found Alembic head `c5d6e7f8a9b0`, 50 public tables, 271 users, 13 organizations, 293 organization memberships, 153 proposals, 0 votes, and 197 delegations. Membership, proposal, vote, and delegation orphan checks were all zero. No row contents, emails, ballot data, private text, tokens, or uploaded files were opened or displayed.
- The signed manifest and extraction both reported zero upload files and zero upload bytes. The temporary destination therefore contained no avatar/logo content and no unexpected top-level entry. This is consistent with an empty logical uploads directory; Railway's 150 MB native volume reference includes volume/filesystem allocation rather than proving application files exist.
- Cleanup is complete: zero decrypted `offsite-restore-*` directories remain; the temporary identity, read-only credential, connection files, extracted uploads directory, downloaded archives/binaries, disposable database service, and orphaned disposable volume were all removed. The retained password-manager and offline key copies were untouched. Both encrypted R2 objects remain private and protected, and the production backend, PostgreSQL, and frontend remained Online.
- The 19:03:31 UTC startup-driven worker backup and distinct 19:04:13 UTC manual run provide independent worker/manual success timestamps and objects, satisfying the normal-worker proof without another schedule change. Final production monitor status is `ok` with zero issues, no offsite failure, and the 58,761,416-byte daily success recorded.
...
- **Stage 6 — DONE.** Production encrypted-backup creation, R2 metadata/lock/lifecycle verification, isolated PostgreSQL 18 restore, aggregate relationship validation, cleanup, credential revocation, and independent worker/manual success proof all passed. Production was never a restore target.
- **Stage 7 — NOT STARTED.** No downgrade-continuity proof has run.
- Both recovery-layer restore proofs are complete. Phase 98 remains operationally open only for Stage 3's clock-bound synthetic lifecycle observation and Stage 7's later Pro-to-Hobby continuity decision/proof.
